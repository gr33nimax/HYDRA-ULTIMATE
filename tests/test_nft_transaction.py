...
def test_snapshot_captures_only_hydra_table_and_policy_route():
    ...
    with patch.object(nft.shutil, "which", return_value="/usr/sbin/tool"), \
         patch.object(nft.subprocess, "run", side_effect=[table, rule]):
...
def test_restore_replaces_only_hydra_table():
    ...
    with patch.object(nft.shutil, "which", return_value="/usr/sbin/nft"), \
         patch.object(nft.subprocess, "run") as run, \
...
def test_restore_empty_snapshot_removes_hydra_policy_only():
    ...
    with patch.object(nft.shutil, "which", return_value="/usr/sbin/nft"), \
         patch.object(nft.subprocess, "run"), \
