...
def test_antidpi_no_longer_resolves_loopback_or_time_correlated_sources():
    """Relay attribution moved out of the collector with the contraction.

    No enabled adapter emits a ``peer_port``, and a time-window guess is not
    attribution, so the collector must not carry those seams at all.
    """
    from hydra.plugins.antidpi import agent

    assert not hasattr(agent, "_resolve_relay_source")
    assert not hasattr(agent, "_resolve_unattributed_relay_source")
...
def test_udp_relay_strips_each_header_and_preserves_bidirectional_flow(tmp_path):
    ...
    with patch("hydra.core.source_relay.MAP_FILE", mapping):
        worker = threading.Thread(
            target=_serve_udp,
            args=(relay, "naive", backend.getsockname()[1]),
            daemon=True,
        )
        worker.start()
