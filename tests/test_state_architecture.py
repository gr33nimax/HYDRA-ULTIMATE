        ...
        for node in ast.walk(_tree(CORE / name)):
            ...
            for module in modules:
                if module.split(".", 1)[0] in forbidden_roots:
                    violations.append(
                        f"{name}:{getattr(node, 'lineno', 0)} {module}",
                    )
    assert violations == []
...
def test_state_storage_facade_reexports_the_pure_schema() -> None:
    ...
    assert len((CORE / "state_models.py").read_text(encoding="utf-8").splitlines()) < 240
    budgets = {
        "state_format.py": 100,
        # One normaliser per legacy format lives here; the budget grows with
        # the formats, not with incidental code.
        "state_migrations.py": 240,
...
def test_state_storage_imports_never_smuggle_domain_types() -> None:
    ...
    for path in paths:
        ...
        for node in ast.walk(_tree(path)):
            if isinstance(node, ast.ImportFrom) and node.module == "hydra.core.state":
                forbidden = {alias.name for alias in node.names if alias.name not in storage_symbols}
                if forbidden:
