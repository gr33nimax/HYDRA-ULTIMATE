"""Host adapter capabilities for the qWDTT VK creator pool."""
...
QWDTT_POOL_DIR = Path("/var/lib/hydra/calls/vk/qwdtt")
QWDTT_POOL_STATE = QWDTT_POOL_DIR / "state.json"
CREATOR_BINARY = Path("/usr/local/bin/headless-vk-creator")
CREATOR_UNIT = Path("/etc/systemd/system/hydra-vk-call-creator@.service")
...
class CallsCreatorInfrastructureMixin:
    """Blue/green creator lifecycle used by ``CallsInfrastructure``."""
    ...
    def install_creator(self) -> tuple[bool, str]:
        ...
        with tempfile.TemporaryDirectory(prefix="hydra-calls-creator-") as work:
    ...
    def creator_units(
        ...
        *,
        ...
        prefix = "wdtt-headless-creator" if legacy else "hydra-vk-call-creator"
        if legacy:
            generation = ""
    ...
    def cleanup_legacy_creator(self) -> tuple[bool, str]:
        failures: list[str] = []
        for unit in self.creator_units(legacy=True):
        ...
        self.host.remove_file(LEGACY_COOKIES_FILE, missing_ok=True)
        self.host.run(["systemctl", "daemon-reload"])
        return True, "legacy WDTT creator removed"
