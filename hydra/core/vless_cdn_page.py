...
def _live_script(data: SiteData) -> str:
    ...
    ошибке замолкает, а запросы плейлиста — это пульс, по которому сторож понимает, что
    зритель ещё здесь: тишина гасит поток, и следующий заход платит холодный старт, пока
    ffmpeg поднимается и режет первый сегмент. Поэтому пинг продлевает жизнь потоку даже
    тогда, когда плеер в ошибке и сам плейлист не опрашивает.
    """
    return (
        "(function () {"
        ...
        "    var fails = 0;"
        "    var session = '';"
        # Пинг — пульс для сторожа: по нему он видит зрителя и не гасит поток, пока
        ...
        "    }"
        # URL сессии берём у самого плеера: details.url — это и есть плейлист. По нему
        # сторож видит зрителя, а не по внутреннему состоянию hls.js.
