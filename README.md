...
- 📦 **Нативная подписка HydraBox.** `?format=hydrabox` отдаёт строгий plaintext
  HydraBox Subscription v1 с remote-safe Sing-Box runtime и явными профилями;
  полный клиентский конфиг по-прежнему доступен через `?format=singbox`.
...
отдельным действием. Интервал автоматического пересоздания настраивается от
1 до 24 часов; значение по умолчанию — 24 часа.
