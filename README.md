...
1. **🌍 Геолокация и провайдер (GeoIP)**
   ...
   * **Какую информацию дает**: Выводит ваш публичный IP-адрес, тип сети, страну, регион, город, а также имя хостинг-провайдера и ASN. Помогает быстро проверить, применился ли нужный регион после настройки VPN/прокси.

2. **🛡️ Доступность ресурсов (Censorcheck)**
...
3. **🛡️ Обход DPI-фильтров (Censorcheck DPI)**
   * **Что делает**: Специальный тест для серверов, расположенных внутри РФ. Проверяет прохождение трафика через ТСПУ (технические средства противодействия угрозам) и DPI-фильтры провайдеров.
