...
В менеджер встроен полноценный раздел отладки и мониторинга работы VPS (`Опция 8` в главном меню). Он позволяет диагностировать сетевые проблемы, проверять геологические блокировки, обход DPI цензуры, а также измерять скорость и производительность системы. Все тесты выполняются неинтерактивно с красивой визуализацией в TUI-рамках.

**Доступные инструменты диагностики:**

1. **🌍 Геолокация и провайдер (GeoIP)**
   * **Что делает**: Опрашивает внешние GeoIP базы данных.
   * **Какую информацию дает**: Выводит ваш публичный IP-адрес, тип сети, страну, регион, город, а также имя хостинг-провайдера и номер автономной системы (ASN). Помогает быстро проверить, применился ли нужный регион после настройки VPN/прокси.

2. **🛡️ Гео-блокировки ресурсов (Censorcheck)**
   * **Что делает**: Тестирует доступность популярных зарубежных ресурсов (таких как OpenAI/ChatGPT, Netflix, YouTube, Spotify, TikTok, GoogleAI и др.) напрямую с вашего сервера.
   * **Какую информацию дает**: Показывает статус доступности каждого сервиса (доступен напрямую, заблокирован хостингом, выдает ошибку 403 или перенаправляет трафик). Позволяет оценить применимость VPS для доступа к заблокированному контенту.

3. **🛡️ Блокировки DPI в РФ (Censorcheck DPI)**
   * **Что делает**: Специальный тест для серверов, расположенных внутри РФ. Проверяет прохождение трафика через ТСПУ (технические средства противодействия угрозам) и DPI-фильтры.
   * **Какую информацию дает**: Тестирует обход блокировок и замедления ресурсов (например, YouTube, Discord, Rutracker) при различных методах обхода DPI, оценивая состояние цензуры на сервере.

4. **⚡ Тест скорости до РФ (iPerf3)**
   * **Что делает**: Выполняет нативный многопоточный замер скорости до опорных серверов iPerf3 в крупнейших городах РФ (Москва, Санкт-Петербург, Нижний Новгород, Челябинск, Тюмень).
   * **Какую информацию дает**: Точные показатели скорости скачивания (Download), выгрузки (Upload) и задержки (Ping) до рунета. Снабжен защитой от зависания (таймаут 6 сек) и автоматическим переключением на резервные узлы (fallback), если основные заняты.

5. **🌐 Тест скорости: Мир (Global Download)**
   * **Что делает**: Нативный Python-загрузчик последовательно тестирует пропускную способность до 14 глобальных дата-центров по всему миру (США, Бразилия, Германия, Италия, Нидерланды, Сингапур, Тайвань, Япония, Австралия).
   * **Какую информацию дает**: Замеряет скорость скачивания и задержку до ключевых международных узлов без лишней нагрузки на трафик (скачивание длится ровно 4 секунды на сервер).

6. **💻 Производительность CPU (Sysbench)**
   * **Что делает**: Проводит нативный стресс-тест процессора VPS в один поток с помощью `sysbench`.
   * **Какую информацию дает**: Выводит количество обработанных событий в секунду (events/sec), общее время выполнения и задержку (минимальную, среднюю, максимальную). Позволяет мгновенно оценить качество процессора, предоставленного хостингом.
