...
При обновлении schema 10 → 15 прежний Calls безопасно остаётся установленным,
но выключается и нормализуется в `vk_parasite`, чтобы общий apply продолжил
обслуживать остальные протоколы. Финальная ступень 14 → 15 сохраняет комнаты и
четыре worker, но выключает несовместимый runtime. После одновременного switch
