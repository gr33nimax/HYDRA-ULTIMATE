...
Безопасный сбор нативных ошибок включённых AntiDPI-транспортов выполняется одной
командой:
...
Она не меняет ключи, постоянную конфигурацию или сервисы, отправляет локальные
некорректные TCP/TLS/UDP-пакеты, запускает временные клиенты с неверной
авторизацией для поддерживаемых транспортов и создаёт закрытый диагностический архив
`/tmp/hydra-antidpi-selftest-*.tar.gz`. Секреты автоматически скрываются, но
архив всё равно следует просмотреть перед передачей. Подробности находятся в
[документации AntiDPI](docs/ANTI_DPI.md).
