...
### v2.3.1 (05.07.2026)
* **Фиксы оркестратора**: Добавлен немедленный отзыв сессий и токенов на сервере подписок при блокировке или удалении пользователя.
* **Установка и деплой**: Исправлены проблемы первого запуска на чистой ОС и авто-миграция старых конфигураций Fail2ban.
* **NaiveProxy**: Формат ссылок подключения изменен на `naive+https://`.

### v2.3.0-beta (04.07.2026)
* **Сервер подписок**: Запущен HTTPS-сервер подписок (порт 9443) с поддержкой формата NekoBox (`sn://`), лимитов трафика и TTL в реальном времени.
* **Новые протоколы**: Интегрированы плагины **Telemt (MTProxy)** для Telegram и **qWDTT** (WireGuard over TURN) с изолированным парольным управлением.
* **VPS-диагностика**: В TUI добавлена панель отладки (тесты GeoIP, скорость iPerf3 до РФ, тесты DPI через Censorcheck и глобальный speedtest).

### v0.5 BETA / v2.2.0 (03.07.2026)
* **Учет трафика**: Реализован фоновый демон `traffic_daemon` для сбора статистики трафика и биллинга пользователей AnyTLS, Mieru и TrustTunnel.
* **SNI Мультиплексор**: Интегрирован **HAProxy** как единая точка входа (порт 443) для распределения запросов по SNI между AnyTLS, NaiveProxy и TrustTunnel.
* **Безопасность**: Добавлены интерактивные TUI-пульты управления для Fail2ban, Honeypot и IPBan.
* **Сеть**: Добавлен модуль выборочного маршрутизации Cloudflare WARP.

### v0.0.3 BETA / v2.1.0 (01.07.2026)
* **Ядро Sing-Box**: Переход на расширенную сборку `sing-box-extended` и перенос **Mieru** на нативный inbound.
* **NaiveProxy**: Интегрирован плагин NaiveProxy на базе Caddy с автоматическим выпуском Let's Encrypt сертификатов.
* **nftables**: Исправлена петля TPROXY-маршрутизации (`meta mark 0xff`).

### v2.0.1-beta (30.06.2026)
* **AmneziaWG 2.0**: Добавлен автоматический бесконфликтный выбор подсетей и ограничение MTU (1376) для стабильности соединения.
* **TUI**: Полный редизайн интерфейса TUI (двойные рамки окон и корректная ширина Emoji/CJK-символов).

### v2.0.0 (29.06.2026)
* **Архитектура v2.0**: Полный рефакторинг проекта. Удалены устаревшие ядра Xray/VLESS. Заложен новый оркестратор конфигов Sing-Box, типизированная схема базы состояний State Schema v2 и модульная структура плагинов.
