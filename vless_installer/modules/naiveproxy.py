...
def _run_install_inner(from_wizard: bool = False) -> None:
    ...
    _box_row()
    _box_sep()
    _box_row(f"  {BOLD}{WHITE}Первый пользователь — данные подключения:{NC}")
    _box_row()
    _box_kv("  Server:",   f"{YELLOW}{domain}{NC}")
    _box_kv("  Port:",     f"{YELLOW}{port}{NC}")
    _box_kv("  Protocol:", f"{YELLOW}HTTPS{NC}")
    _box_kv("  SNI:",      f"{YELLOW}{domain}{NC}")
    _box_kv("  Username:", f"{YELLOW}{users[0]['username']}{NC}")
    _box_kv("  Password:", f"{YELLOW}{users[0]['password']}{NC}")
    _box_row()
    _box_sep()
    _box_info("Рекомендуемый способ — sing-box JSON (пункт [6] меню):")
    _box_info("работает в Karing, Exclave и NekoBox без правок.")
    _box_info("naive+https:// ссылка не распознаётся как share-link")
    _box_info("большинством клиентов — используйте её на свой риск,")
    _box_info("либо вводите поля выше вручную (тип 'NaiveProxy').")
...
def _show_singbox_json(domain: str, port: int, username: str, password: str) -> None:
    ...
    _box_row()
    _box_info("Работает в Karing, Exclave и NekoBox через импорт")
    _box_info("конфига / 'Import from Clipboard' — вставьте код ниже.")
    _box_row()
    _box_bot()
    print()
    print(json_str)
...
def _guide_clients() -> None:
    ...
    _box_row()
    _box_sep()
    _box_row(f"  {BOLD}{WHITE}Рекомендуемый способ — sing-box JSON:{NC}")
    _box_row()
    _box_info("Пункт [6] главного меню → скопировать JSON →")
    _box_info("вставить в Karing / Exclave / NekoBox как импорт")
    _box_info("конфига или 'Import from Clipboard'.")
    _box_row()
    _box_sep()
    _box_row(f"  {BOLD}{WHITE}Ручной ввод (если клиент не принял JSON):{NC}")
    _box_row()
    _box_kv("  Server:",   domain, 12)
    _box_kv("  Port:",     str(port), 12)
    _box_kv("  Protocol:", "HTTPS", 12)
    _box_kv("  SNI:",      domain, 12)
    _box_row()
    _box_sep()
    _box_row(f"  {BOLD}{WHITE}Клиенты:")
    _box_row()
    _box_kv("  Karing",        "iOS / Android / Windows / macOS / Linux", 16)
    _box_kv("  Exclave",       "Android — выбрать тип 'NaiveProxy'", 16)
    _box_kv("  NekoBox",       "Android — выбрать тип 'NaiveProxy'", 16)
    _box_kv("  naiveproxy",    "CLI — все платформы", 16)
    _box_row()
    _box_sep()
    _box_warn("naive+https:// ссылка не распознаётся как share-link")
    _box_warn("большинством клиентов — используйте JSON или ручной ввод.")
    _box_bot(); _pause()
