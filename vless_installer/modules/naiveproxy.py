...
def _run_install_inner() -> None:
    ...
    _box_row()
    _box_sep()
    _box_row(f"  {BOLD}{WHITE}Первый пользователь — данные для ручного ввода:{NC}")
    _box_row()
    ...
    _box_row()
    _box_sep()
    _box_info("У NaiveProxy нет единого стандарта share-ссылок —")
    _box_info("вводите поля выше вручную при добавлении профиля.")
    _box_info("Клиенты: NekoBox, husi, Exclave (Android) — выбрать")
    _box_info("тип 'NaiveProxy' и заполнить поля как указано выше.")
    _box_warn("Karing требует sing-box JSON — см. пункт [6] в меню.")
