...
def _write_config(port, ipv4, ipv6, tls_domain, users, use_middle_proxy,
                  ...
                  client_mss: str = "") -> None:
    """
    Генерирует /etc/telemt/telemt.toml.
    ...
    Аргументы:
      ...
      client_mss       →  пресет MSS ("tspu", "2in8", числовой или "").

    ВАЖНО — порядок секций TOML (фиксированный):
      [general] → [general.modes] → [general.links]
      → [server]              ← client_mss здесь, ДО [[server.listeners]]
      → [[server.listeners]]  ← открытие array-of-tables закрывает [server]
      → [timeouts] → [censorship] → [access] → [access.users]
      → [[upstreams]] → [dc_overrides] → [middle_proxy]

    В TOML первое вхождение [[server.listeners]] закрывает таблицу [server].
    Любой ключ server.* ПОСЛЕ [[server.listeners]] игнорируется парсером —
    именно так client_mss не отрабатывал при вставке через list.insert().
    """
    ...
    lines = [
        ...
        # me2dc_fallback/me2dc_fast: встроенный fallback ME→Direct внутри бинарника.
        # Дополняет Python-оркестратор telemt_fallback.py, не конфликтует с ним.
        # Применяется только при use_middle_proxy = true.
        "me2dc_fallback     = true",
        "me2dc_fast         = true",
        # Явные значения, чтобы не зависеть от дефолтов при обновлениях Telemt.
        'log_level          = "normal"',
        "disable_colors     = true",
    ...
    # ── [server] ──────────────────────────────────────────────────────────────
    # ВСЕ ключи [server] (включая client_mss) должны быть здесь,
    # ДО первого [[server.listeners]]. После него [server] закрыт.
    lines += [
        "",
        "[server]",
        f"port = {port}",
    ...
    if client_mss:
        # Тип всегда String: числа тоже в кавычках ("256", "tspu", "2in8").
        lines.append(f'client_mss = "{client_mss}"')

    # ── [[server.listeners]] — открывает array-of-tables, закрывает [server] ─
    lines.append("")
    if ipv4:
    ...
    # ── [timeouts] ────────────────────────────────────────────────────────────
    # client_handshake: таймаут фазы TLS-рукопожатия (не idle-соединения).
    # Дефолт Telemt = 30с. Старое значение 300 позволяло зондам держать
    # TCP-соединение открытым 5 минут без завершения handshake.
    # relay_idle_policy_v2: управляет долгоживущими idle-сессиями.
    # tg_connect: таймаут TCP-соединения к серверам Telegram.
    lines += [
        "[timeouts]",
        "client_handshake                                 = 30",
        "client_keepalive                                 = 15",
    ...
    # ── [censorship] ──────────────────────────────────────────────────────────
    # tls_new_session_tickets = 1: TLS 1.3 NewSessionTicket — часть нормального
    #   handshake реального HTTPS-сервера. Без него отпечаток прокси отличается.
    # server_hello_delay_{min,max}_ms: случайный джиттер в ServerHello.
    #   ТСПУ анализирует тайминги TLS. Должен быть < client_handshake * 1000.
    # tls_full_cert_ttl_secs: как долго кэшировать cert для одного IP.
    #   3600с снижает частоту повторных рукопожатий с одного адреса.
    lines += [
        "[censorship]",
        ...
        "mask_port                 = 443",
        "fake_cert_len             = 2048",
        "tls_new_session_tickets   = 1",
        "tls_full_cert_ttl_secs    = 3600",
        "server_hello_delay_min_ms = 30",
    ...
    # ── [dc_overrides] — только в Direct Mode ─────────────────────────────────
    ...
    # ── [middle_proxy] — дописывается отдельно через append_fallback_section ──
