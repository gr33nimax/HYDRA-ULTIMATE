...
def do_hysteria2_menu() -> None:
    ...
    while True:
        ...
        # ── Конфигурация ──────────────────────────────────────────────────────
        _box_row()
        _box_item("1", f"💻  Exit-нода           {DIM}Установка/управление H2 сервером{NC}")
        _box_item("2", f"🔀 Выбор транспорта     {DIM}AWG / Hysteria2 / Оба + веса{NC}")
        _box_item("3", f"⚡  Балансировщик нод    {DIM}Стратегия, веса, автопереключение{NC}")
        _box_row()
        _box_sep()
        ...
        _box_item("8", f"🔺  Обновление           {DIM}Автообновление бинарника H2{NC}")
