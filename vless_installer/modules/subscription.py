...
ВАЖНО, что нужно знать перед использованием:
  ...
  1. Домен подписки — ОТДЕЛЬНЫЙ от Reality/NaiveProxy, со своим A-record.
     ...
     проверяет, свободен ли порт (включая 80, нужный Caddy для ACME
     HTTP-01), и предлагает альтернативный (например 8443), если занят.
...
def _write_caddyfile(domain: str, port: int, email: str = "") -> None:
    _PUBLIC_DIR.mkdir(parents=True, exist_ok=True)
    global_block = f"{{\n    email {email}\n}}\n\n" if email else ""
    content = (
        f"{global_block}"
...
def _run_setup_inner() -> None:
    ...
    try:
        ...
        if port == 80 or _port_in_use(80) is False:
            pass  # порт 80 нужен Caddy для ACME HTTP-01; просто не мешаем, не трогаем

        email = _ask(f"  {CYAN}Email для Let's Encrypt (опционально, Enter — пропустить): {NC}",
    ...
    _CFG_DIR.mkdir(parents=True, exist_ok=True)
    _write_caddyfile(domain, port, email)
    print(f"  {GREEN}✓{NC}  Caddyfile записан: {_CADDYFILE}")
    _install_service()
    print(f"  {GREEN}✓{NC}  Systemd-сервис создан.")

    _run(["systemctl", "restart", _SERVICE_NAME])
    print(f"  {CYAN}→{NC}  Жду выпуск сертификата (до 20 секунд)...")
    time.sleep(5)

    r = _run(["systemctl", "is-active", _SERVICE_NAME], capture=True)
    svc_ok = r.stdout.strip() == "active"

    journal = _run(["journalctl", "-u", _SERVICE_NAME, "-n", "60", "--no-pager"], capture=True)
    log_text = journal.stdout or ""
    cert_ok = ("certificate obtained successfully" in log_text
               or "certificate is already being managed" in log_text)
    cert_err = ("failed to get certificate" in log_text.lower()
                or "challenge failed" in log_text.lower())
    ...
    if cert_ok:
        _box_ok("Сертификат Let's Encrypt получен.")
    elif cert_err:
        _box_err("Ошибка выпуска сертификата — проверьте, что A-запись")
        _box_err("домена указывает на этот сервер и порт 80 свободен.")
    else:
        _box_warn("Статус сертификата неясен — посмотрите логи (пункт «Статус»).")
    _box_row()
