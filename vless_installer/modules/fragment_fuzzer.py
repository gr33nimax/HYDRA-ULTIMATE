...
def do_fragment_fuzzer_menu() -> None:
    ...
    _box_desc(
        "Проверяет доступность VPS через разные параметры фрагментации: "
        "перебирает комбинации length/interval/packets и измеряет TLS Handshake. "
        "Тест идёт с VPS, поэтому результат ориентировочный — для точного "
        "подбора используйте F4 (генерация всех пресетов) и тестируйте "
