...
> **Модуль:** `antidpi` · **Служба:** `hydra-antidpi` · **Введён:** 2.5.1-dev «FORTRESS» · **Актуален в:** 2.5.5
...
- JSONL-tails переживают truncate и rotation; после rename-rotation новый файл
  читается с начала, а недописанная строка ждёт завершения вместо потери;
- один фильтрованный journal worker читает Caddy, Sing-Box, protocol units,
  ...
  сохранённый cursor `--after-cursor` исключает пропуск событий во время
  простоя службы; cursor подтверждается только после атомарной записи улики в
  AntiDPI state, поэтому переполнение очереди или рестарт дают безопасный
  повтор, а не потерю события;
- очередь ограничена и при переполнении сохраняет наиболее свежие события;
- state обновляется под file lock и записывается атомарно; при старте и каждые
  10 минут коллектор сам восстанавливает ipset-наборы, правила и активные баны,
а также снимает баны, покрытые whitelist. Reconciliation восстанавливает оба
ipset, DROP-правила, scan/Mieru telemetry и сохранённые баны; последний
результат и незавершённые шаги видны в health, TUI и Telegram.
...
Такая атрибуция по времени — только подсказка для наблюдения: событие получает
`ban_eligible = false` и может привести к ALERT, но никогда само — к бану.
Бан требует точного соответствия `protocol + relay-port` конкретного
соединения.
...
Решающий `port_sweep` достижим только для подтверждённого прикладными журналами
адреса: kernel-телеметрия (см. раздел 9) принципиально не повышает
`ban_eligible`, сколько бы портов ни накопила.

Порядок применения бана транзакционен относительно state: сначала запись о бане
сохраняется на диск (durable intent), затем адрес добавляется в ipset. Если
процесс упал между шагами — старт коллектора возвращает бан в firewall; если
ipset отказал — запись откатывается, отказ фиксируется в `ban_failures`, и
адрес остаётся под наблюдением для повтора.

Время записи события используется для decay и корреляции, а время выполнения
решения — для firewall. Событие из backlog получает только остаток исходного
TTL; если к моменту обработки score уже распался ниже порога или TTL истёк,
новая блокировка не создаётся.
...
То же верно для kernel-телеметрии TCP SYN/NEW: LOG-правило фиксирует пакет, но
не доказывает, что отправитель владеет source-адресом. Поэтому любые события
`kernel-firewall` получают `ban_eligible = false` независимо от числа
накопленных портов.
...
kernel SYN/NEW evidence -> observed score -> ALERT -> никогда автоматический BAN
```
...
| HTTPS decoy | активный запрос к scanner/decoy path (сопоставляется нормализованный путь, а не весь URI с query) | немедленный BAN |
| AnyTLS | auth failure, EOF до первого пакета | ALERT + BAN |
| TrustTunnel TCP/QUIC | auth/authorization failure (только явный auth-reject 401/407; 5xx и ошибки destination — не улики) | ALERT + BAN |
| ShadowTLS | HMAC mismatch (alert-only при атрибуции по времени), malformed TLS, Trojan auth failure | ALERT + BAN |
...
Ошибки из общего журнала Sing-Box сначала сопоставляются специфичным
протокольным паттернам и только потом — generic-правилу, поэтому линия
Hysteria2 или AnyTLS сохраняет собственный протокол, атрибуцию и UDP-политику.
...
формируют `port_sweep` — сигнал остаётся alert-only, пока адрес не подтверждён
прикладным журналом.
...
Если какой-то из снимаемых банов не удалось убрать, операция возвращает ошибку
с перечнем адресов, а старт коллектора повторяет снятие — «успех» никогда не
означает «доверенный адрес остался заблокирован».
...
доставки; бан-уведомления внутри пятисекундного cooldown пропускаются и
показываются отдельным счётчиком.

Доставка уведомлений выполняется отдельным worker через ограниченную очередь:
медленный или недоступный Telegram API не задерживает обработку улик и
административные операции. Переполнение очереди учитывается как failed и
отдельно как dropped, поэтому dashboard не выдаёт потерю за доставку.
...
правами `0600`, read-modify-write защищён `flock`. Повреждённый файл не
маскируется под пустое состояние: он переносится в
`antidpi.json.corrupt-<время>` для диагностики. До восстановления валидного
state детектор переходит в degraded и не применяет автоматические блокировки,
чтобы неизвестный прежний whitelist не был потерян; состояние видно в health,
TUI и Telegram. Healthcheck также фиксирует застрявший сборщик по
`collector_heartbeat_at` (записывается раз в минуту; 10 минут тишины при
активной службе — неисправность).
...
Ограничение списка не влияет на карточку адреса: запрос `address_details`
возвращает точную запись одного IP (активный бан, наблюдение, последние записи
history) независимо от того, попал ли адрес в усечённый watchlist. Карточка
различает «улик нет» и «нет данных» — недоступный источник не отображается
как благополучный.
...
| Раздел | Назначение |
| :--- | :--- |
| Блокировки и история | Баллы, остаток срока, причина, источник, offense; постраничный список, разбан по номеру видимой строки или адресу; массовый разбан требует подтверждения точного списка |
| Под наблюдением | Адреса с уликами ниже порога блокировки, прогресс-бар score и доля подтверждённых улик |
| Whitelist | Доверенные адреса и подсети; добавление снимает их баны |
| Заблокировать адрес | Бессрочный ручной бан до снятия оператором, с подтверждением адреса |
| Журнал Anti-DPI | Последние 50 строк журнала службы целиком, с подсветкой ошибок и банов |
...
`[REDACTED]`. Значения чувствительных ключей (включая `Authorization`,
`Proxy-Authorization`, `Cookie`, `password`) удаляются структурно — целиком,
для JSON-объектов и массивов, а не только первое слово заголовка. Перед
передачей архив всё равно необходимо просмотреть:
