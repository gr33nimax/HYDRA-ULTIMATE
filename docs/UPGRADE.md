...
3  read-only preflight новым кодом     работает, трафик идёт
   ─────────────────── точка невозврата ───────────────────
4  запомнить активные units,           службы HYDRA остановлены
   остановить процессы HYDRA           sing-box и прокси продолжают работу
5  снимок сырого state + wrapper,      службы HYDRA остановлены
   проверенный backup-архив
6  миграция схемы state                службы HYDRA остановлены
7  переключение симлинка /opt/hydra    службы HYDRA остановлены
