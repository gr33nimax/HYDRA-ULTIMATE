...
При включённом WDTT общий apply сверяет `/etc/wdtt/server-revision` с
зафиксированной ревизией `gr33nimax/hydra-wdtt`, а не с плавающей веткой
upstream. Отсутствующий или устаревший маркер означает legacy-бинарник: сервер
пересобирается и заменяется атомарно до перезапуска службы. Тот же apply создаёт
`/etc/wdtt/hydra-access.json`; файл содержит только verifier-данные HydraBox
grants и имеет права `0600`.
