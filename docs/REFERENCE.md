...
тогда транспорт становился бы неустановимым. SHA-256 digest из release metadata обязателен и
проверяется до распаковки; аварийный флаг `HYDRA_ALLOW_UNVERIFIED_DOWNLOADS` на этот путь не
...
сохраняются при любой ошибке. Сбой сообщается своей стадией — выбор релиза, проверка digest,
распаковка/ELF, сервисный пользователь или пути, юнит, служба, маршрутизация — вместо общего
