...
из `/etc/hydra/warp_profiles`. Отсутствующая цель считается ошибкой конфигурации
...
`sudo hydra plugin reinstall warp` удаляет файлы прежнего установщика
(`/usr/local/bin/wgcf`, `wgcf-profile.conf`, `wgcf-account.toml`,
`/var/log/hydra/warp_install.log`), заново скачивает каталог и списки и
сохраняет настроенные маршруты; то же удаление доступно пунктом
«🧹 Убрать остатки wgcf» в меню WARP.

Списки правил берутся из каталога Geo-Aggregator (`db/catalog.json`), а не из
зашитого набора: 314 источников в 20 категориях, обновление ежедневное. Каталог
кэшируется в `/var/lib/hydra/warp_catalog.json` и обновляется той же задачей
обслуживания, что и сами списки, поэтому сервис, добавленный upstream, доходит
до оператора без релиза HYDRA. Если каталог ещё не скачан, плагин отвечает
встроенной копией (`refilter`, `antifilter`, `category-ru`, `category-ai`).

Оператор маршрутизирует **категорию**, а не отдельный источник: меню пишет один
target всем спискам категории, а отдельный список можно переопределить внутри
неё (страницы по 20, `[n]`/`[p]`). Если источники категории разошлись (например,
обновление каталога добавило новый список), меню показывает «разные
направления» вместо одного из них. Направление указано в самом меню, потому что
ошибка тут неочевидна: категории `ru` нужен российский адрес (`direct`),
а `blocked` — иностранный (`warp`), иначе сайты просто не откроются. Источник
`category-ru` дополнительно тянет за собой TLD `.ru`, `.su`, `.рф`,
`.xn--p1ai`. Ключи прежних списков (`ext:russia`, `ext:geoblock`,
`ext:google_ai`) переименовываются в `ext:category-ru`, `ext:refilter`,
`ext:category-ai` при загрузке state, выбранный target сохраняется.
