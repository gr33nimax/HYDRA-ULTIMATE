...
накопительный счётчик. Недоступность API (выключен, не-200, неразобранное тело, отсутствие
пользователя, таймаут) не считается нулём: экран мониторинга показывает «источник недоступен»,
последние достоверные накопленные значения сохраняются, а причина видна в статусе и health Telemt.
