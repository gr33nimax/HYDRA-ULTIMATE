...
Сводные `category-*`, `itDog-*`, `refilter`, `antifilter` исключены при загрузке
каталога и из меню. Каталог кэшируется в `/var/lib/hydra/warp_catalog.json` и
обновляется ежедневно. До первой успешной загрузки внешние сервисы недоступны;
локальный список доменов WARP продолжает работать без сети.

В меню можно направить всю категорию **отдельных сервисов** или выбрать один
сервис внутри неё (поиск и страницы по 20, `[n]`/`[p]`). Выбор `none` убран:
