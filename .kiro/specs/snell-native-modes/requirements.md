...
| --- | --- | --- |
| server (`inbound`) | `version` 5 (`obfs_mode: none,http` — `tls` exists only in generations 1–3, and no client answers it on 5) or 6 (`mode: default,unshaped,unsafe-raw`); 4 is refused | `snell: unsupported version: 4`, `Snell obfs mode must be none or http` |
...
4. WHEN the generation is `5` THEN the operator SHALL be able to choose an obfuscation mode
   `none` or `http` and an obfuscation host used by `http` (`tls` belongs to generations 1–3,
   which this core cannot serve).
