...
### Explicit TLS multiplexer audit

Добавлена read-only проверка Caddy L4/TLS-маршрутов. `hydra doctor` теперь
сверяет ожидаемые SNI-домены из state с фактическим `tls_mux`-конфигом,
выявляет пропавшие и устаревшие маршруты, проверяет сертификаты и состояние
`caddy-l4`. Результат также включён в `hydra plan` и `hydra status` в блоке
`tls_mux`. Проверка не перезапускает сервис и не меняет конфигурацию; для
исправления рассинхронизации используется обычный транзакционный `hydra apply`.

### Honeypot apply stability

Повторное применение конфигурации теперь не перезапускает уже здоровый Honeypot, если его сгенерированный скрипт не изменился. Это предотвращает блокировку включения другого протокола (например, Mieru) из-за транзитного сбоя лишнего restart-а Honeypot; реальные изменения его настроек по-прежнему применяются с проверкой стабильности.

Honeypot формально переведён в standalone lifecycle (`central_apply=False`): общий Sing-Box apply больше не применяет его вообще, а enable/disable и изменения настроек остаются в собственном транзакционном lifecycle. Исправлен также сгенерированный runtime-скрипт, который ошибочно ссылался на недоступный объект `HOST` вместо локального bounded `subprocess.run`.
