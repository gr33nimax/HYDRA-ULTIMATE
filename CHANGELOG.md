...
- HydraBox Subscription обновлена до v2: WDTT endpoint содержит только
  `credential_ref`, а device grant передаётся исключительно внутри JWE
  `credentials`; сброс устройств и ротация JWE-ключа транзакционно отзывают
  выданные WDTT grants.
...
- Сервер обновлён до ревизии с `worker-takeover-v1`: переподключение после смены
  Wi-Fi/мобильной сети атомарно заменяет прежний worker-slot, не расходует квоту
  повторно и не требует обновления HydraBox-подписки. SIGHUP сохраняет действующие
  lease-ключи и учёт worker'ов; отзыв или смена binding по-прежнему закрывает сессии.

- Общий apply теперь сверяет `/etc/wdtt/server-revision` с закреплённой ревизией
  `hydra-wdtt` и автоматически заменяет ранее установленный legacy-бинарник.
  Замена выполняется атомарно; при последующем сбое общей транзакции прежний
  бинарник и маркер ревизии восстанавливаются из plugin snapshot.
- Сервер собирается из зафиксированной ревизии форка `gr33nimax/hydra-wdtt` и
  принимает verifier-only `/etc/wdtt/hydra-access.json` с O(1) key hints,
  15-минутными leases и обновлением после 10 минут через VK relay.
- Политика подписки задаёт 9 воркеров минимум, 18 по умолчанию и 36 максимум;
  горячая ротация получает временную дополнительную группу из 9. Автоматическая
  глобальная квота масштабируется по числу device bindings без фиксированного
  лимита пользователей, сохраняя явные ресурсные квоты VPS/relay.
