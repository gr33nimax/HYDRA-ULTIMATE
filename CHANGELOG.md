...
- WARP больше не ставит внешний установщик профиля. Транспорт переведён на нативный `masque`
  ...
  первом применении. Переключение существующей установки — `sudo hydra plugin reinstall warp`:
  он удаляет файлы прежнего установщика (`/usr/local/bin/wgcf`, `wgcf-profile.conf`,
  `wgcf-account.toml`, `/var/log/hydra/warp_install.log`), заново скачивает каталог и списки и
  сохраняет настроенные маршруты; то же удаление доступно пунктом «🧹 Убрать остатки wgcf» в
  меню WARP.
