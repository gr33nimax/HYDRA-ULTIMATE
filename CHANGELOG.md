...
- TUI Headless Creator разделён на три коротких экрана: сам creator, VK cookies
  и qWDTT-комнаты. Меню Calls теперь показывает только действия, допустимые для
  текущего состояния транспорта.
