...
- Мониторинг трафика больше не показывает несуществующие нули. Telemt читает per-user накопительный
  счётчик `total_octets` из control API upstream (`GET /v1/stats/users`), который Hydra рендерит
  включённым только на loopback (`127.0.0.1:9091`, whitelist `127.0.0.0/8`); MTProto Zig отличает
  «HTTP 200 без поддерживаемых серий» от настоящего нуля. Недоступность источника (API выключен,
  не-200, неразобранное тело, отсутствие пользователя, таймаут) показывается как «источник
  недоступен», последние достоверные накопленные значения сохраняются, а причина видна в статусе и
  health протокола. Устаревшая схема `stats.json` (iptables-цепочки, cron, оценка доли по сессиям)
  не возвращается. Таблица «По протоколам» выровнена по 77-клеточной раскладке (16/12/23/13/8),
  паддинг считается по видимой ширине без ANSI, ненулевая доля всегда рисует хотя бы один блок,
  подписи берут продуктовые имена (`Telemt`, `AnyTLS`) при отсутствии явного display name, а
...
- Ручные конфигурации больше не повторяют уже показанные MTProto `tg://proxy`-ссылки JSON-обёрткой
  `{"link": ..., "protocol": ...}`; сама копируемая ссылка Telemt/MTProto Zig остаётся на месте.
...
- CI rollback-тест Telemt теперь изолирует его рабочий каталог в `tmp_path`; production-пути и
  runtime-поведение не менялись.

- Меню Telemt больше не содержит дублирующего пункта «Показать ссылки»: per-user ссылки
  Telegram выдаются только в общем экране ручных конфигураций пользователя, который агрегирует
  артефакты всех включённых транспортов через `ApplicationService.protocols.client_links()`.
  Вывод credentials, формат `tg://proxy`-ссылки, фильтрация заблокированных пользователей,
  подписки и ссылки остальных транспортов не менялись; у выключенного Telemt остаётся общий пустой
  экран без отдельного fallback. Остальные действия меню сохранили смысл и получили непрерывную
  нумерацию.

- Расширенные настройки Telemt стали коротким списком из трёх строк — привязка сети, MiddleProxy и
  уровень логов, — каждая с текущим значением и честным следствием. Сохранение строки меняет ровно
  одну настройку и возвращает в тот же список, отмена строки ничего не сохраняет, `0` выходит назад,
  а недопустимое значение отклоняется с названием поля. Нормальная установка по-прежнему спрашивает
  только порт и TLS-домен.
