...
- Sing-Box-подписка сохраняет единственный составной клиентский профиль
  атомарно. В частности, ShadowTLS больше не теряет DNS и связь
  `Trojan -> ShadowTLS` при запросе `?format=singbox`.
- AmneziaWG desktop/mobile включаются в `?format=singbox` как endpoints
  `wireguard` для sing-box-extended с ключами, peer, MTU и полным набором
  параметров обфускации `amnezia`; нативная `.conf`-выдача не изменилась.
